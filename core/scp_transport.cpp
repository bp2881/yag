...
// ============================================================================
static std::string ssh_prefix(const std::string& host,
...
std::string ssh_read_file(const std::string& host,
    ...
    std::string cmd = ssh_prefix(host, user, port)
                    + " \"cat '" + remote_path + "'\"";

    auto [output, exit_code] = exec_command(cmd);
    if (exit_code != 0) {
        throw std::runtime_error(
            "ssh_read_file failed (exit " + std::to_string(exit_code)
            + "): " + remote_path);
    }
...
void ssh_mkdir(const std::string& host,
    ...
    if (exit_code != 0) {
        throw std::runtime_error(
            "ssh_mkdir failed (exit " + std::to_string(exit_code)
            + "): " + remote_dir);
    }
}
...
void ssh_write_file(const std::string& host,
    ...
    std::string cmd = "echo '" + encoded + "' | "
                    + ssh_prefix(host, user, port)
                    + " \"base64 -d > '" + remote_path + "'\"";

    auto [output, exit_code] = exec_command(cmd);
    if (exit_code != 0) {
        throw std::runtime_error(
            "ssh_write_file failed (exit " + std::to_string(exit_code)
            + "): " + remote_path);
    }
}
...
void scp_upload(const fs::path& local_path,
                const std::string& host,
                const std::string& user,
                int port,
                const std::string& remote_path) {
    std::string cmd = scp_prefix(port)
                    + " '" + local_path.string() + "'"
    ...
    if (exit_code != 0) {
        throw std::runtime_error(
            "scp_upload failed (exit " + std::to_string(exit_code)
...
void scp_download(const std::string& host,
    ...
    if (exit_code != 0) {
        throw std::runtime_error(
            "scp_download failed (exit " + std::to_string(exit_code)
...
void scp_upload_missing(const fs::path& local_dir,
                        const std::string& host,
                        const std::string& user,
                        int port,
                        const std::string& remote_dir) {
    if (!fs::exists(local_dir)) return;
