...
static std::string
build_metadata(const std::string &parent, const std::string &branch,
...
std::string create_commit(const std::string &message) {
  ...
  if (parent != "none") {
    ...
    std::map<std::string, std::string> parent_map(parent_commit.files.begin(),
                                                  parent_commit.files.end());
    std::map<std::string, std::string> index_map(file_entries.begin(),
                                                 file_entries.end());
  ...
  std::string metadata =
      build_metadata(parent, branch, timestamp, message, file_entries);
    ...
    if (in_files) {
      ...
      if (line.size() > 2 && line[0] == ' ' && line[1] == ' ') {
        ...
        if (space != std::string::npos) {
          commit.files.emplace_back(entry.substr(0, space),
