...
std::string get_remote_host() { return get_config_value("remote_host"); }

std::string get_remote_user() { return get_config_value("remote_user"); }
...
void set_remote_spec(const std::string &spec) {
  ...
  if (at_pos == std::string::npos) {
    throw std::runtime_error("Invalid remote spec: '" + spec +
  ...
  if (user.empty() || host.empty()) {
    throw std::runtime_error("Invalid remote spec: '" + spec +
                             "'. User and host must be non-empty.");
  }
...
bool init(const std::string &project_name, const std::string &remote_spec) {
  ...
  if (fs::exists(yag_path)) {
    std::cerr << "Repository already initialized in "
  ...
  std::cout << "Initialized empty YAG repository in " << yag_path.string()
            << "\n";
